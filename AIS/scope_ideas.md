...
- Phase I activities/deliverables:
  ...
  - HIL feasibility plan with COSMIAC prototype compute board; edge resource budget and thermal map. [R5][R6]
...
- Concept: COSMIAC reconfigurable passive RF front‑end (SDR/FPGA) with on‑sensor feature extraction; IA onboard ML for maneuver/intent cues and compression for comms‑denied ops. [R2][R6]
...
- Phase I: sensor trade studies; reconfig profiles; tasking policies; on‑sensor compute budget. [R5]
...
- Space MLOps & V&V: pipeline for dataset curation, synthetic data, active learning, performance monitoring, certifiable on‑orbit updates, secure rollback. [R3][R5][R6]
- HIL/Prototype Test Plan: bench test outline at COSMIAC (power/thermal/SEU injection/SDR), with data capture for Phase II gating. [R5]
...
- Thread A (Edge+Bus): POLARIS policy + autonomy manager + safe action broker, with HIL feasibility on a rad‑tolerant SoC/FPGA concept. [R5][R6]
- Thread B (Sensors+Edge): Passive RF micro‑payload with on‑sensor features and triage; compression + tip‑and‑cue to reduce downlink. [R2][R6]
